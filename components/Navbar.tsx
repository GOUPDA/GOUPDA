...
const Navbar: React.FC<NavbarProps> = ({ isScrolled, brandName }) => {
  ...
  const categories = ['Collection', 'Heritage'];
  ...
  return (
    <nav className={`transition-all duration-500 ${isScrolled ? 'py-4 border-b border-stone-100 shadow-sm' : 'py-8'}`}>
      <div className="max-w-[1440px] mx-auto px-6 md:px-12">
        <div className="flex items-center justify-between relative">
          ...
          <div className="flex-1 hidden md:flex items-center">
             <a href="#" className={`text-[10px] uppercase tracking-[0.4em] font-medium transition-colors ${isScrolled ? 'text-stone-500 hover:text-stone-900' : 'text-white/80 hover:text-white'}`}>
               {categories[0]}
             </a>
          </div>
          ...
          <div className="flex-none">
            <h1 className={`text-2xl md:text-4xl tracking-[0.5em] font-light transition-colors serif ${isScrolled ? 'text-stone-900' : 'text-white'}`}>
              {brandName}
            </h1>
          </div>
      ...
      <div className={`fixed inset-0 bg-stone-900/40 backdrop-blur-sm z-[100] transition-opacity duration-500 ${isMenuOpen ? 'opacity-100 visible' : 'opacity-0 invisible'}`} onClick={() => setIsMenuOpen(false)}>
        <div 
          className={`absolute right-0 top-0 bottom-0 w-[80%] bg-white p-8 transition-transform duration-500 ease-out ${isMenuOpen ? 'translate-x-0' : 'translate-x-full'}`}
          onClick={e => e.stopPropagation()}
        >
          <div className="flex justify-between items-center mb-12">
            <span className="text-xl tracking-[0.3em] font-light serif">{brandName}</span>
